package com.example.ui.theme

import androidx.compose.ui.graphics.Color

val Purple80 = Color(0xFFD0BCFF)
val PurpleGrey80 = Color(0xFFCCC2DC)
val Pink80 = Color(0xFFEFB8C8)

val Purple40 = Color(0xFF6750A4)
val PurpleGrey40 = Color(0xFF625B71)
val Pink40 = Color(0xFF7D5260)

// Bento Theme Palette Colors
val BentoBg = Color(0xFFFAF9FD)
val BentoCardBg = Color(0xFFFEF7FF)
val BentoPrimary = Color(0xFF6750A4)
val BentoPrimaryContainer = Color(0xFFEADDFF)
val BentoOnPrimaryContainer = Color(0xFF21005D)
val BentoSecondaryContainer = Color(0xFFE8DEF8)
val BentoOnSecondaryContainer = Color(0xFF1D192B)
val GoldPremium = Color(0xFFD4AF37)

// Bento Theme Specific Palette
val BentoBackground = Color(0xFFFAF9FD)
val BentoTextPrimary = Color(0xFF1C1B1F)
val BentoPurple = Color(0xFF6750A4)
val BentoLightPurple = Color(0xFFEADDFF)
val BentoVeryLightPurple = Color(0xFFFEF7FF)
val BentoAccentPurple = Color(0xFFE8DEF8)
val BentoBorder = Color(0xFFE6E1E5)
val BentoBorderDark = Color(0xFFD0BCFF)
val BentoBorderNeutral = Color(0xFFCAC4D0)
val BentoNavBg = Color(0xFFF3EDF7)
